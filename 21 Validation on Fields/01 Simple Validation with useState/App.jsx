...
export default function App() {
  ...
  let passwordRegex = /^(?=.{8,}$)(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[^A-Za-z0-9]).*$/;
  ...
  function updatePassword (value) {
    ...
    if(value == ""){
      ...
      setPasswordError("Password Must satisfy this conditions :: at least one uppercase, one lowercase, one digit, and one symbol");
    }else {
      setPasswordError("");
      setPassword(value);
    }
  ...
  return (
    <div style={{ textAlign: "center" }}>
      ...
      <input className={nameError?'error':''} type="text" placeholder="Enter User Name" onChange={(event)=>updateName(event.target.value)}/>
      <br />
      {nameError? <span>{nameError}</span>: null}
      <br /><br />
      <input className={passwordError?'error':''} type="text" placeholder="Enter User Password" onChange={(event)=>updatePassword(event.target.value)}/>
      <br />
